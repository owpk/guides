/*
 * This file is generated by jOOQ.
 */
package nu.studer.sample.tables.pojos;


import java.io.Serializable;
import java.time.LocalDateTime;
import java.util.UUID;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class SmartexamRequirementsForSpItems_U74375 implements Serializable {

    private static final long serialVersionUID = 1L;

    private final UUID          id;
    private final Integer       version;
    private final LocalDateTime createTs;
    private final String        createdBy;
    private final LocalDateTime updateTs;
    private final String        updatedBy;
    private final LocalDateTime deleteTs;
    private final String        deletedBy;
    private final UUID          nomenclatureId;
    private final String        currentStage;
    private final UUID          requirementsForSpId;

    public SmartexamRequirementsForSpItems_U74375(SmartexamRequirementsForSpItems_U74375 value) {
        this.id = value.id;
        this.version = value.version;
        this.createTs = value.createTs;
        this.createdBy = value.createdBy;
        this.updateTs = value.updateTs;
        this.updatedBy = value.updatedBy;
        this.deleteTs = value.deleteTs;
        this.deletedBy = value.deletedBy;
        this.nomenclatureId = value.nomenclatureId;
        this.currentStage = value.currentStage;
        this.requirementsForSpId = value.requirementsForSpId;
    }

    public SmartexamRequirementsForSpItems_U74375(
        UUID          id,
        Integer       version,
        LocalDateTime createTs,
        String        createdBy,
        LocalDateTime updateTs,
        String        updatedBy,
        LocalDateTime deleteTs,
        String        deletedBy,
        UUID          nomenclatureId,
        String        currentStage,
        UUID          requirementsForSpId
    ) {
        this.id = id;
        this.version = version;
        this.createTs = createTs;
        this.createdBy = createdBy;
        this.updateTs = updateTs;
        this.updatedBy = updatedBy;
        this.deleteTs = deleteTs;
        this.deletedBy = deletedBy;
        this.nomenclatureId = nomenclatureId;
        this.currentStage = currentStage;
        this.requirementsForSpId = requirementsForSpId;
    }

    /**
     * Getter for
     * <code>public.smartexam_requirements_for_sp_items__u74375.id</code>.
     */
    public UUID getId() {
        return this.id;
    }

    /**
     * Getter for
     * <code>public.smartexam_requirements_for_sp_items__u74375.version</code>.
     */
    public Integer getVersion() {
        return this.version;
    }

    /**
     * Getter for
     * <code>public.smartexam_requirements_for_sp_items__u74375.create_ts</code>.
     */
    public LocalDateTime getCreateTs() {
        return this.createTs;
    }

    /**
     * Getter for
     * <code>public.smartexam_requirements_for_sp_items__u74375.created_by</code>.
     */
    public String getCreatedBy() {
        return this.createdBy;
    }

    /**
     * Getter for
     * <code>public.smartexam_requirements_for_sp_items__u74375.update_ts</code>.
     */
    public LocalDateTime getUpdateTs() {
        return this.updateTs;
    }

    /**
     * Getter for
     * <code>public.smartexam_requirements_for_sp_items__u74375.updated_by</code>.
     */
    public String getUpdatedBy() {
        return this.updatedBy;
    }

    /**
     * Getter for
     * <code>public.smartexam_requirements_for_sp_items__u74375.delete_ts</code>.
     */
    public LocalDateTime getDeleteTs() {
        return this.deleteTs;
    }

    /**
     * Getter for
     * <code>public.smartexam_requirements_for_sp_items__u74375.deleted_by</code>.
     */
    public String getDeletedBy() {
        return this.deletedBy;
    }

    /**
     * Getter for
     * <code>public.smartexam_requirements_for_sp_items__u74375.nomenclature_id</code>.
     */
    public UUID getNomenclatureId() {
        return this.nomenclatureId;
    }

    /**
     * Getter for
     * <code>public.smartexam_requirements_for_sp_items__u74375.current_stage</code>.
     */
    public String getCurrentStage() {
        return this.currentStage;
    }

    /**
     * Getter for
     * <code>public.smartexam_requirements_for_sp_items__u74375.requirements_for_sp_id</code>.
     */
    public UUID getRequirementsForSpId() {
        return this.requirementsForSpId;
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder("SmartexamRequirementsForSpItems_U74375 (");

        sb.append(id);
        sb.append(", ").append(version);
        sb.append(", ").append(createTs);
        sb.append(", ").append(createdBy);
        sb.append(", ").append(updateTs);
        sb.append(", ").append(updatedBy);
        sb.append(", ").append(deleteTs);
        sb.append(", ").append(deletedBy);
        sb.append(", ").append(nomenclatureId);
        sb.append(", ").append(currentStage);
        sb.append(", ").append(requirementsForSpId);

        sb.append(")");
        return sb.toString();
    }
}
